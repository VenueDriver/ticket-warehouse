SELECT
    ...
    CAST(ticket_data.price AS DECIMAL(10, 2)) AS price_per_ticket,
    CAST(ticket_data.surcharge AS DECIMAL(10, 2)) AS surcharge_per_ticket,
    CAST(ticket_data.let_tax AS DECIMAL(10, 2)) AS let_per_ticket,
    CAST(ticket_data.venue_fee AS DECIMAL(10, 2)) AS service_charge_per_ticket,
    CAST(ticket_data.sales_tax AS DECIMAL(10, 2)) AS sales_tax_per_ticket,
     COUNT(*) AS sold,
    SUM(CAST(ticket_data.price AS DECIMAL(10, 2))) AS face_value_admissions_total,
    SUM(CAST(ticket_data.let_tax AS DECIMAL(10, 2))) AS let_total,
    SUM(CAST(ticket_data.venue_fee AS DECIMAL(10, 2))) AS service_charge_total,
    SUM(CAST(ticket_data.surcharge AS DECIMAL(10, 2))) AS surcharge_total,
    SUM(CAST(ticket_data.sales_tax AS DECIMAL(10, 2))) AS sales_tax_total
FROM
